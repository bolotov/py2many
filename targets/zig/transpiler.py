...
from .inference import get_inferred_zig_type
from .plugins import (
    ATTR_DISPATCH_TABLE,
...
class ZigTranspiler(CLikeTranspiler):
    ...
    def headers(self, meta=None):
        return "\n".join([self._import(h) for h in self._headers])
    ...
    def _combine_value_index(self, value_type, index_type) -> str: # WARNING: Signature of "_combine_value_index" incompatible with supertype "py2many.clike.CLikeTranspiler"
    ...
    def visit_FunctionDef(self, node) -> str:
        ...
        typenames, args = self.visit(node.args) # WARNING: Unpacking a string is disallowed

        args_list = []
        if len(args) and hasattr(node, "self_type"): #WARNING: Cannot determine type of "args"
            typenames[0] = node.self_type  #WARNING: Cannot determine type of "typenames"

        for i in range(len(args)):  #WARNING: Cannot determine type of "args"
            typename = typenames[i]    #WARNING: Cannot determine type of "typenames"
            arg = args[i]  #WARNING: Cannot determine type of "args"
...
#         return f"{try_prefix} {fname}({args})"
    ...
    def visit_While(self, node) -> str:
        buf = [f"while ({self.visit(node.test)}) {{"]
        buf.extend(
            [self.indent(self.visit(n), level=node.level + 1) for n in node.body]
        )
        return "\n".join(buf) + "\n}"
    ...
    def visit_Bytes(self, node) -> str:
        return repr(node.value).replace("'", '"') # WAS: node.s
    ...
    def visit_ClassDef(self, node) -> str:
        extractor = DeclarationExtractor(ZigTranspiler())
        ...
        struct_def = f"{zig_decorators}struct {node.name}:\n"
    ...
    def visit_StrEnum(self, node) -> str: # FIXME: is it redundant? Is it wrong?
        fields = []
        for member, var in node.class_assignments.items():
            var = self.visit(var)
            if var == "auto()":
    ...
    def _import_from(self, module_name: str, names: List[str], level: int = 0) -> str:
        ...
        if len(names) == 1:
            ...
            if lookup in MODULE_DISPATCH_TABLE:
                zig_module, zig_name = MODULE_DISPATCH_TABLE[lookup]
                return f"const {zig_name} = @import(\"{zig_module}\").{zig_name};"
        return f"// unsupported import from {module_name}: {names}"
